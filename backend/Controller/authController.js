  ...
  static registration = async (req, res) => {
    ...
    try {
      if (name && email && phone && password) {
        ...
        if (isUser) {
          return res.status(208).json({ message: "User Already found" });
        } else {
          // //password Hashing
        ...
        return res.status(205).json({ message: "All Field are required" });
      }
    } catch (e) {
      ...
      if (email && password) {
        ...
        if (isUser) {
          if (
            ...
            return res.status(207).json({ message: "Invalid Credentials" });
          }
        } else {
          return res.status(208).json({ message: "User Not found" });
        }
      } else {
        return res.status(206).json({ message: "All Fields are required" });
      }
    } catch (e) {
      console.log(e.message);
    }
